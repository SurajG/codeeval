...
def possible_values digits
  return [] if digits.empty?
  return [digits.to_i] if digits.size == 1

  (1..(digits.size)).map do |i|
    num = digits[0,i].to_i
    exprs = possible_values digits[i..-1]
...
File.open(ARGV[0]).map(&:strip).each do |digits|
  puts possible_values(digits).count { |x| ugly? x }
end
