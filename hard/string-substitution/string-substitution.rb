...
# alphabet { 0, 1 }. Searching should consider only contiguous pieces of S that
# have not been subject to replacements on prior iterations. An iteration of the
# algorithm should not write over any previous replacement by the algorithm.
#
# Input sample:
#
...
# => 11100110 [replacing 10 with 11] => 11100110}

def replace str, replacements
  return str if replacements.empty?
  rep = replacements.first
  reps = replacements[1..-1]

  index = str.index rep[0]
  if index
    (index == 0 ? '' : replace(str[0..index-1], reps)) + rep[1] +
      replace(str[index+rep[0].size..-1], replacements)
  else
    replace(str, reps)
  end
end
...
File.open(ARGV[0]).map(&:strip).each do |line|
  data = line.split ';'
  str = data[0]
  replacements = data[1].split(',')
  replacements = (0..replacements.size/2 - 1).map do |i|
    [replacements[2*i], replacements[2*i + 1]]
  end

  puts replace str, replacements
end
