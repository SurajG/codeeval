...
# in the segment. So a segment of keys of length 2 is terminated by 11. The
# entire encoded message is terminated by 000 (which would signify a segment in
# which the keys have length 0). The message is decoded by translating the keys
# in the segments one-at-a-time into the header characters to which they have
# been mapped.
# Input sample:
#
...
# keys will map to that character. The encoded message contains only 0's and
# 1's, and it is a legitimate encoding according to the described scheme. That
# is, the message segments begin with the 3-digit length sequence and end with
# the appropriate sequence of 1's. The keys in any given segment are all of the
# same length, and they all correspond to characters in the header. The message
# is terminated by 000.
# Your program should accept the first argument as the filename and read the
# contents of this file as the test data, according to the conditions above.eg.
#
...
def build_keymap header
  ...
  keymap << curmap unless curmap.empty?
...
def decode header, message
  ...
  decoded = ""
  while (key_size = message.slice!(/.{3}/).to_i(2)) > 0
    while (key = message.slice!(/.{#{key_size}}/)).include? '0' do
      decoded += keymap[key_size - 1][key.to_i(2)]
    end
  end
...
ARGF.map{ |line| line.match(/\d+/) { |m| [m.pre_match, m.to_s] } }.each do |h,m|
  puts decode(h, m)
end
